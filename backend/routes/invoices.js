...
const { calculateInterest, getRbiBankRate, getRateHistory } = require('../services/interest.js')
const { createNotification } = require('../services/notify.js')
const { daysOverdue, stageForDays, STAGES } = require('../services/escalation.js')
const { generateNoticePDF } = require('../services/pdf.js')
...
function validateInvoice(body, isPatch) {
  ...
  for (const key of Object.keys(body)) {
    if (!ALLOWED_FIELDS.includes(key)) errors[key] = `Unknown field: "${key}"`
  }
  if (!isPatch) {
...
router.get('/transactions', wrapAsync(async (req, res) => {
  ...
  const enriched = data.map(inv => {
    ...
    const { totalInterest, breakdown } = calculateInterest(inv.amount, inv.deliveryDate, inv.agreedTermsDays, paidAt, bankRate, rateHistory)
...
router.get('/interest', wrapAsync(async (req, res) => {
  const now = new Date()
...
router.get('/:id', wrapAsync(async (req, res) => {
  ...
  if (!invoice || invoice.msmeId !== req.msmeId) return res.status(404).json({ error: 'not found' })
  res.json(invoice)
}))

router.get('/:id/timeline', wrapAsync(async (req, res) => {
  const invoice = await Invoice.findById(req.params.id)
  if (!invoice || invoice.msmeId !== req.msmeId) return res.status(404).json({ error: 'not found' })
  const now = new Date()
...
router.get('/:id/notice', wrapAsync(async (req, res) => {
  const invoice = await Invoice.findById(req.params.id)
  if (!invoice || invoice.msmeId !== req.msmeId) return res.status(404).json({ error: 'not found' })
...
router.get('/:id/breakdown', wrapAsync(async (req, res) => {
  const invoice = await Invoice.findById(req.params.id)
  if (!invoice || invoice.msmeId !== req.msmeId) return res.status(404).json({ error: 'not found' })
  const [bankRate, rateHistory] = await Promise.all([getRbiBankRate(), getRateHistory()])
  const { totalInterest, breakdown } = calculateInterest(invoice.amount, invoice.deliveryDate, invoice.agreedTermsDays, new Date(), bankRate, rateHistory)
  res.json({
...
router.patch('/:id', wrapAsync(async (req, res) => {
  ...
  if (errors) return res.status(400).json({ error: 'Validation failed', fields: errors })
  if (!picked || Object.keys(picked).length === 0) return res.status(400).json({ error: 'No valid fields to update' })
